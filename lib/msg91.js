 ...
 *   MSG91_WA_NAMESPACE               — WhatsApp Business Account namespace (from template dashboard)
...
/** Internal: send a WhatsApp template message via MSG91 (to_and_components format) */
async function _sendWhatsApp({ mobile, templateName, namespace, variables = [] }) {
  const key  = authKey();
  const from = waNumber();
  if (!key || !from) {
  ...
  // Build body_N components object: { body_1: {type,value}, body_2: ... }
  const components = {};
  variables.forEach((v, i) => {
    components[`body_${i + 1}`] = { type: 'text', value: String(v) };
  });

  const ns = namespace || process.env.MSG91_WA_NAMESPACE || '';
...
export async function sendWhatsAppDonation({ mobile, name, amount, donationId, paymentId }) {
  ...
  return _sendWhatsApp({
    mobile,
    templateName: process.env.MSG91_WA_DONATION_TEMPLATE || 'thank_you_for_donation_fwf',
 ...
 * Template: donation_receipt_sms  |  Vars: ##name## ##amount##
 */
 ...
 * Template: donation_receipt_80g_sms  |  Vars: ##name## ##amount##
 */
